...
export function getAddressByLnglat(lnglat, city = '全国', radius = 1000, callback) {
...
export function getLngLatByAddress(keyword, city = '全国', callback) {
  return new Promise((resolve, reject)=>{
 ...
 */
export function transformCoord(lnglat, source) {
  const _SOURCE = 'WGS84';
  const _TARGET = 'AMap';
...
function initTools(AMapU) {
  ...
  AMapU.prototype.getAddressByLnglat = getAddressByLnglat;

  AMapU.prototype.getLngLatByAddress = getLngLatByAddress;

  AMapU.prototype.transformCoord = transformCoord;
